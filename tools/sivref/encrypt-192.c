/*   
     ...
     SIV mode by: Rhys Weatherley
*/   
 
#include "crypto_aead.h"
#include <string.h>

#define FrameBitsIV1 0x90  
#define FrameBitsIV2 0xB0  
#define FrameBitsAD  0x30  
#define FrameBitsPC1 0x50  //Framebits for plaintext/ciphertext      
#define FrameBitsPC2 0xD0
...
void initialization(const unsigned char *key, const unsigned char *iv, unsigned int *state, unsigned char domain)
{
	int i;
	...
	)
{
	...
	unsigned int state[4];   
        unsigned char npub2[12];

	//initialization stage
	initialization(k, npub, state, FrameBitsIV1);
	...
	*clen = mlen + 8; 
        ...
        //re-initialize
        memcpy(npub2, npub, 4);
        memcpy(npub2 + 4, mac, 8);
        initialization(k, npub2, state, FrameBitsIV2);

	//encrypt the plaintext    
	for (i = 0; i < (mlen >> 2); i++)
	{
		state[1] ^= FrameBitsPC2;  
		state_update(state, k, NROUND2); 
	...
	)
{
	...
	*mlen = clen - 8; 
        ...
        //initialization stage to decrypt the contents
        memcpy(npub2, npub, 4);
        memcpy(npub2 + 4, c + *mlen, 8);
        initialization(k, npub2, state, FrameBitsIV2);
        ...
        //initialization stage to authenticate the contents
	initialization(k, npub, state, FrameBitsIV1);
